...
> We have had to add the `as.character()` function around `input$date_input` otherwise it will return the time in  [Unix Epoch](https://en.wikipedia.org/wiki/Unix_time) format.
...
## [**Exercise 1**](02_inputs-Answer_key.md#exercise-1)
...
Also, to enhance the widget inputs, [shinyWidgets](https://github.com/dreamRs/shinyWidgets) is an R package that you can install that gives you even more stylistic options for your Shiny app. The gallery of input widgets that you can create using `shinyWidgets` can be found by using the command:

```
# Bring up the shinyWidgets Gallery
shinyWidgets::shinyWidgetsGallery()
```

A useful part of the gallery is that each input widget displayed has a `</> Show code` section that gives you the code needed to create the input widget on the UI side.
