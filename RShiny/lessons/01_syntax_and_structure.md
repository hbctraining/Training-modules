...
Each Shiny App has 3 main component:
...
2) **Server** - This sectio provides the instructions for how input data will be processed and returned by the app. This is referred to as the _back-end_ of the app
...
When you are running an app your console will be unavailible. In order to get your console back you need to either close the app or press the red stop sign in the top right of the console. If you relaunch the app (by re-running the `shinyApp` function), you can view the app in a web browser by:

1) Clicking "Open in Browser" at the topof the the app window
...
We will discuss various input and output types at length in the next lesson, but this line is creating a text input in the UI where the input is being saved to a variable called `input_text`. This variable will be sent to the server as `input$input_text. "My input text" is the text above the input text box. Because this is the first line of code for the app, this input text box will appear at the top of the app.

```
textOutput("output_text")
```

This is telling the UI where to put the output text after it has been rendered by the server. We haven't seen where the variable `output_text` is created yet, but we will see it in the server section below. 
...
The server function is created with the object `input` and `output` to hold the input and output of the app. 
...
Now that we've gone through each line, let's talk about what happens when we run the `shinyApp()` fucntion to tie it all together. 
